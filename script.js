/*!
 * WA Aqua GestÃ£o
 * Copyright Â© 2026 Matheus. Todos os direitos reservados / All rights reserved.
 * Uso, cÃ³pia, modificaÃ§Ã£o ou distribuiÃ§Ã£o proibidos sem autorizaÃ§Ã£o por escrito.
...
function _obterTaxa(peso) {
  ...
  // Acima de 30 g usa a taxa do maior peso da tabela (nÃ£o trava a estimativa).
...
async function toggleMenuUsuario() {
  ...
  const nome = user.user_metadata?.nome || user.email?.split("@")[0] || "UsuÃ¡rio";
...
// Trava um botÃ£o de aÃ§Ã£o durante uma operaÃ§Ã£o de rede: mostra spinner + texto,
// bloqueia duplo toque e devolve uma funÃ§Ã£o para restaurar o estado original.
// Use no topo da funÃ§Ã£o: if (botao?.disabled) return;  (antes de qualquer await)
...
// Gera um identificador Ãºnico para vincular lanÃ§amentos ao ciclo correto
      ...
      <div class="form-lancamento">
        ...
        <div class="form-corpo">
          <div class="campo-form">
            ...
            <input type="text" id="inputNomePerfil" placeholder="Ex: Fazenda SÃ£o JoÃ£o" value="${nome}">
          </div>
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">SeguranÃ§a</h2>
      </div>
      <div class="form-corpo">
        <div class="campo-form">
          <div class="campo-label">
          ...
          <input type="password" id="inputNovaSenha" placeholder="MÃ­nimo 6 caracteres">
        </div>
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
...
async function salvarSenha() {
  ...
  if (!nova || nova.length < 6) { _erroSenha("A senha deve ter no mÃ­nimo 6 caracteres."); return; }
  if (nova !== confirmar) { _erroSenha("As senhas nÃ£o coincidem."); return; }
...
async function sairUsuario(botao) {
  ...
  viveiros = [];
  // replace(): Voltar/AvanÃ§ar nÃ£o devem reabrir o app autenticado apÃ³s o logout
...
async function abrirConfiguracoes() {
  ...
  area.innerHTML = `
    <h3 class="titulo-secao">ConfiguraÃ§Ãµes</h3>
    <div class="cfg-wrap">
      ...
      <div class="cfg-lista">
        ...
        ${_cfgItem("seguranca", "SeguranÃ§a", "Alterar senha", "abrirSeguranca()")}
        ${_cfgItem("aparencia", "AparÃªncia", "Tema claro ou escuro", "abrirAparencia()")}
        ${_cfgItem("conta", "Minha conta", "Plano e assinatura", "abrirMinhaConta()")}
        ${_cfgItem("faq", "Perguntas frequentes (FAQ)", "DÃºvidas sobre o sistema", "abrirFAQ()")}
      ...
      <button class="cfg-item cfg-item-sair" onclick="confirmarSairConta()">
        ...
        <div class="cfg-item-texto">
          ...
          <span class="cfg-item-sub">Encerrar sessÃ£o atual</span>
        </div>
      </button>
...
// ExclusÃ£o da prÃ³pria conta: apaga todos os dados no servidor (Edge Function
// excluir-conta, com service role) e encerra a sessÃ£o.
async function excluirMinhaConta(botao) {
  ...
  try {
    ...
    if (error || (data && data.error)) {
      ...
      _toastErro("NÃ£o foi possÃ­vel excluir a conta. Tente de novo ou fale com o suporte.");
      restaurar();
      return;
    }
    ...
    <div class="cfg-wrap">
      ...
      <div class="form-corpo" style="padding:0">
        <div class="campo-form">
          ...
          <input type="text" id="fzNome" value="${nome}" placeholder="Ex: Fazenda SÃ£o JoÃ£o">
        </div>
        <div class="campo-form">
          <div class="campo-label"><svg class="campo-icone" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><label>Nome do proprietÃ¡rio</label></div>
...
async function salvarFazenda() {
  ...
  if (emailMudou) {
    ...
    if (e2) { setMsg("Dados salvos, mas o e-mail nÃ£o pÃ´de ser alterado: " + e2.message); return; }
    _toastSucesso("Enviamos um link de confirmaÃ§Ã£o para o novo e-mail.");
  } else {
    _toastSucesso("AlteraÃ§Ãµes salvas!");
  }
  atualizarAvatarTopo();
  abrirConfiguracoes();
}
...
function abrirSeguranca() {
  ...
  const area = document.getElementById("area-gestao");
  area.innerHTML = `
    <h3 class="titulo-secao">SeguranÃ§a</h3>
    <div class="cfg-wrap">
      <div class="cfg-hero">
        ...
        <h4>Alterar senha</h4>
        <p>Para sua seguranÃ§a, escolha uma senha forte.</p>
      </div>
      <div class="form-corpo" style="padding:0">
        <div class="campo-form">
        ...
        <div class="campo-form">
          ...
          <input type="password" id="segNova" placeholder="MÃ­nimo 6 caracteres">
        </div>
        <div class="campo-form">
...
async function salvarNovaSenha() {
  ...
  if (!nova || nova.length < 6) { setMsg("A nova senha deve ter no mÃ­nimo 6 caracteres."); return; }
  if (nova !== conf) { setMsg("As senhas nÃ£o coincidem."); return; }
...
function abrirAparencia() {
  ...
  const area = document.getElementById("area-gestao");
  area.innerHTML = `
    <h3 class="titulo-secao">AparÃªncia</h3>
    <div class="cfg-wrap">
      <p class="cfg-secao-desc">Escolha como o aplicativo deve aparecer. A mudanÃ§a Ã© aplicada imediatamente.</p>
  ...
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      ...
      <div class="conta-info">
        ...
        <div class="conta-info-linha"><span>PrÃ³xima cobranÃ§a</span><strong>${proxStr}</strong></div>
      </div>
      <div class="cfg-lista">
      ...
      <div class="conta-aviso">
        ...
        <span>Sua assinatura garante acesso a todos os recursos do WA Aqua GestÃ£o.</span>
      </div>
      <button class="botao-voltar-form" style="margin-top:14px" onclick="abrirConfiguracoes()">Voltar</button>
    </div>
  `;
...
const _FAQ = [
  ...
  { q: "Como lanÃ§ar o consumo de raÃ§Ã£o?", a: "Abra o viveiro e toque em LanÃ§ar raÃ§Ã£o. Informe a quantidade fornecida no dia e confirme o lanÃ§amento. O consumo serÃ¡ somado ao histÃ³rico do cultivo e utilizado nos cÃ¡lculos de biomassa, FCA e demais indicadores do sistema." },
];
  ...
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      <div class="faq-busca">
        ...
        <input type="text" id="faqBusca" placeholder="Buscar dÃºvidas..." oninput="filtrarFAQ(this.value)">
      </div>
      <div class="faq-lista" id="faq-lista">
        ${_FAQ.map((f, i) => _faqItem(f, i)).join("")}
        <p id="faq-vazio" class="faq-vazio" style="display:none">Nenhuma dÃºvida encontrada. Fale com o suporte.</p>
      </div>
      <button class="botao-voltar-form" style="margin-top:14px" onclick="abrirConfiguracoes()">Voltar</button>
    </div>
  `;
...
function abrirSuporte() {
  ...
  const txtProblema = encodeURIComponent("OlÃ¡! Quero reportar um problema no WA Aqua GestÃ£o:");
  const txtSugestao = encodeURIComponent("OlÃ¡! Tenho uma sugestÃ£o para o WA Aqua GestÃ£o:");
  const area = document.getElementById("area-gestao");
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      ...
      <div class="cfg-lista">
        ...
        <a class="cfg-item" href="https://wa.me/${wa}?text=${txtSugestao}" target="_blank" rel="noopener">
          ...
          <div class="cfg-item-texto"><span class="cfg-item-titulo">Enviar sugestÃ£o</span><span class="cfg-item-sub">Conte sua ideia para a gente</span></div>
          ${chev}
        </a>
      </div>
      <button class="botao-voltar-form" style="margin-top:14px" onclick="abrirConfiguracoes()">Voltar</button>
...
async function pegarUsuarioLogado() {
  ...
  if (error || !user) {
    _toastErro("Sua sessÃ£o expirou. FaÃ§a login novamente.");
    return null;
  }
...
function formatarMoedaBlur(input) {
  ...
  if (!v) return;
  // pt-BR: ponto Ã© separador de milhar, vÃ­rgula Ã© decimal (sempre)
...
// Converte "YYYY-MM-DD" (ou Date) para um Date Ã  meia-noite LOCAL,
// evitando o deslocamento de fuso de new Date("YYYY-MM-DD") (que Ã© UTC).
...
// Mostra/esconde a setinha de voltar no topo (Ãºtil no iPhone em modo app,
// onde nÃ£o hÃ¡ botÃ£o/gesto de voltar do sistema).
function _toggleVoltarTopo(mostrar) {
  const b = document.getElementById("btn-voltar-topo");
  if (b) b.style.display = mostrar ? "flex" : "none";
}

// Volta UMA tela (mesma aÃ§Ã£o do "Voltar" do celular). Usado pela setinha do topo.
function irParaTelaAnterior() {
  const btn = _voltarBotaoVisivel();
  if (!btn) { voltarMenuGestao(); return; }
  // Se hÃ¡ proteÃ§Ã£o de histÃ³rico armada, usa history.back() para manter tudo
  // sincronizado (dispara o mesmo fluxo do voltar do celular); senÃ£o, clica direto.
...
function _voltarBotaoVisivel() {
  ...
  // Ãšltimo botÃ£o visÃ­vel (o "Voltar" costuma ficar no rodapÃ© da tela).
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        ...
        <div class="cad-modo-toggle">
          <button type="button" class="cad-modo-btn ativo" id="cadBtnPrep" onclick="_cadModo('prep')">Em preparaÃ§Ã£o</button>
        ...
        <div id="cad-prep">
          <div class="campo-form">
            <div class="campo-label">
              ...
              <label>InÃ­cio da preparaÃ§Ã£o</label>
            </div>
        ...
        <div id="cad-cultivo" style="display:none">
          ...
          <div class="campo-form">
            <div class="campo-label">
              ...
              <label>LaboratÃ³rio (fornecedor de pÃ³s-larva)</label>
            </div>
...
async function salvarViveiro() {
  ...
  // Gate de criaÃ§Ã£o: respeita o limite de viveiros do plano.
  const limite = _planoLimiteEfetivo();
  if (viveiros.length >= limite) {
    _toastErro(limite <= 1
      ? "O plano grÃ¡tis permite 1 viveiro. Assine um plano em \"Meu plano\" para cadastrar mais."
      : `Seu plano permite ${limite} viveiros. FaÃ§a upgrade em \"Meu plano\" para cadastrar mais.`);
    return;
  }
  ...
  // Trava: nÃ£o permite dois viveiros com o mesmo nome
  const nomeNorm = nome.trim().toLowerCase();
  if (viveiros.some(v => (v.nome || "").trim().toLowerCase() === nomeNorm)) {
    mostrarErroViveiro(`JÃ¡ existe um viveiro chamado "${nome.trim()}". Use outro nome.`);
    return;
  }

  // Monta o registro conforme o modo (valida os campos especÃ­ficos)
  let novoViveiro;
  if (modo === "prep") {
    const dataPrep = document.getElementById("dataPreparacao").value;
    if (!dataPrep) { mostrarErroViveiro("Informe a data de inÃ­cio da preparaÃ§Ã£o."); return; }
    ...
    if (!data || !total || !laboratorio) { mostrarErroViveiro("Preencha data de povoamento, total povoado e laboratÃ³rio."); return; }
    novoViveiro = { nome, tamanho, ativo: true, data_povoamento: data, total_povoado: total, laboratorio, data_preparacao: null };
  }

  // Feedback visual imediato (< 50 ms) e trava do botÃ£o
  ...
  novoViveiro.ciclo_id = _novoCicloId(); // vincula os lanÃ§amentos deste ciclo
  ...
  // Mostra a lista jÃ¡ posicionada no viveiro recÃ©m-criado
...
function mostrarListaViveiros(posicao = 0, direcao = "", msg = "") {
  ...
  const navProximo = posicao < total - 1
    ? `<button class="botao-nav-viveiro" onclick="mostrarListaViveiros(${posicao + 1}, 'proximo')">PrÃ³ximo</button>`
    : `<span class="botao-nav-viveiro" style="visibility:hidden">PrÃ³ximo</span>`;
    ...
    <div class="viveiro-card">
      ...
      <div class="vc-info-lista">
        <div class="vc-info-item">
          <div class="vc-info-icone verde">ğŸ—“ï¸</div>
          <div>
        ...
        <div class="vc-info-item">
          <div class="vc-info-icone azul">ğŸ§ª</div>
          <div>
            <strong>LaboratÃ³rio</strong>
  ...
  // Mensagem de sucesso (ex: apÃ³s excluir viveiro)
  if (msg) {
  ...
  area.addEventListener("touchend", e => {
    // SÃ³ swipa se ainda estiver na tela de lista de viveiros
...
function abrirViveiro(index) {
  ...
  // Se o viveiro nÃ£o tem ciclo ativo, mostra tela de novo ciclo
  ...
  // Fonte Ãºnica: custos manuais do ciclo (inclui legados sem ciclo_id na janela)
  // + custo fixo rateado, desde a preparaÃ§Ã£o/povoamento atÃ© hoje.
  ...
  // Ãšltima biometria e mÃ©dia de crescimento
  ...
  // SobrevivÃªncia estimada e FCI estimado
  ...
  if (populacaoNum && ultimaRacaoNaoZero && pesoUltimaBio) {
    ...
    if (res && res.biomassa > 0) {
      // res.biomassa = biomassa atual no viveiro, estimada pela raÃ§Ã£o que os
      // camarÃµes remanescentes estÃ£o comendo AGORA (jÃ¡ Ã© pÃ³s-despesca).
      const biomassaAtual = res.biomassa;
      const remanescentes = res.quantidade; // = biomassaAtual / (peso/1000)
      // SobrevivÃªncia conta remanescentes + jÃ¡ despescados (despesca nÃ£o Ã© mortalidade)
      const sobreviventes = remanescentes + despQtdTotal;
      // NÃƒO limita a 100%: mostrar 103% + alerta Ã© melhor que esconder a inconsistÃªncia
      ...
      // ProjeÃ§Ã£o final = remanescentes crescendo atÃ© a meta + o que jÃ¡ foi despescado
    ...
    <div class="painel-viveiro">
      ...
      <div class="painel-info">
        ...
        <div class="info-box">
          ...
          <small>MÃ©dia de crescimento</small>
        ...
        <div class="info-box">
          ...
          <small>SobrevivÃªncia est.</small>
      ...
      <div class="vv-secao-lbl">AÃ§Ãµes de administraÃ§Ã£o</div>
      ...
      <div id="confirmar-excluir-viveiro-${index}" style="display:none;margin:0 16px 10px;background:#fff5f5;border:1px solid #fca5a5;border-radius:10px;padding:9px 11px">
        ...
        <p style="margin:0 0 7px;font-size:10.5px;color:#7f1d1d;line-height:1.3">Os dados serÃ£o desativados (recuperÃ¡veis pelo suporte).</p>
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">RaÃ§Ãµes</h2>
      </div>
      <div class="form-corpo">
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">Cadastrar RaÃ§Ã£o</h2>
      </div>
      <div class="form-corpo">
        <div class="campo-form">
          <div class="campo-label">
...
async function salvarTipoRacao() {
  ...
  if (botao?.disabled) return; // trava contra duplo toque (evita raÃ§Ãµes duplicadas)
  ...
  if (!nome) { mostrarErroTipoRacao("Digite o nome da raÃ§Ã£o."); return; }
  ...
  // Trava contra nome duplicado (mesmo nome = jÃ¡ existe; mude o nome para cadastrar outra)
  const nomeNorm = nome.toLowerCase();
  if (tiposRacao.some(t => (t.nome || "").trim().toLowerCase() === nomeNorm)) {
    mostrarErroTipoRacao(`JÃ¡ existe uma raÃ§Ã£o chamada "${nome}". Use outro nome ou edite a existente.`);
    return;
  }
  ...
  if (error) {
    ...
    mostrarErroTipoRacao(
      error.code === "42P01"
        ? "Tabela 'tipos_racao' nÃ£o existe. Execute o SQL no Supabase primeiro."
    ...
    area.innerHTML = `
      <div class="form-lancamento">
        <div class="form-topo">
          ...
          <h2 class="form-titulo">RaÃ§Ãµes Cadastradas</h2>
        </div>
        <div class="form-corpo">
          <div class="viveiro-sem-ciclo-msg">
            <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            <span>Nenhuma raÃ§Ã£o cadastrada ainda.</span>
          </div>
          <button class="botao-voltar-form" onclick="abrirRacoesCatalogo()">Voltar</button>
        </div>
      </div>
...
async function excluirTipoRacao(i, botao) {
  ...
  if (emUso) {
    _toastErro("Esta raÃ§Ã£o tem lanÃ§amentos no ciclo atual e nÃ£o pode ser excluÃ­da (o custo dos lanÃ§amentos zeraria).");
    return;
  }
  const restaurar = _travarBotao(botao, "Excluindo...");
  const usuario = await pegarUsuarioLogado();
  ...
  _montarCustoRacaoVirtual(); // catÃ¡logo mudou custo derivado atualiza
  abrirVerTiposRacao();
}
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">Editar RaÃ§Ã£o</h2>
      </div>
      <div class="form-corpo">
        <div class="campo-form">
          <div class="campo-label">
...
async function salvarEdicaoTipoRacao(i) {
  ...
  _montarCustoRacaoVirtual(); // preÃ§o mudou custo derivado atualiza na hora
  abrirVerTiposRacao();
}
  ...
  if (viveiros.length === 0) {
    area.innerHTML = `
      <div class="resultado-box">
        ...
        <span>Cadastre um viveiro antes de lanÃ§ar raÃ§Ã£o.</span>
      </div>
    `;
    return;
  }
    ...
    area.innerHTML = `
      <div class="form-lancamento">
        <div class="form-topo">
          ...
          <h2 class="form-titulo">LanÃ§ar RaÃ§Ã£o</h2>
        </div>
        <div class="viveiro-sem-ciclo-msg">
          <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          <span>Nenhum viveiro com ciclo ativo. Inicie um ciclo antes de lanÃ§ar raÃ§Ã£o.</span>
        </div>
        <button class="botao-voltar-form" onclick="voltarMenuGestao()">Voltar</button>
      </div>
    `;
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        ...
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
            <label>Consumo de raÃ§Ã£o</label>
          </div>
          <div class="campo-input-unidade">
...
async function salvarLancamentoRacao(indexDireto = "") {
  ...
  if (!data || isNaN(racao) || racao < 0) {
    mostrarErroRacao("Preencha a data e a quantidade (pode ser 0 para dia sem raÃ§Ã£o).");
    return;
  }

  // Verifica se jÃ¡ existe lanÃ§amento nessa data (normaliza formato)
  const jaExiste = (viveiros[index].racoes || []).some(r => r.data.substring(0, 10) === data);
  if (jaExiste) {
    mostrarErroRacao(`JÃ¡ existe um lanÃ§amento em ${formatarData(data)}. Edite o lanÃ§amento existente.`);
    return;
  }

  // Feedback imediato + trava do botÃ£o (antes de qualquer await)
  const restaurar = _travarBotao(botao, "Salvando...");
  ...
  // Protegido: nunca pode quebrar o lanÃ§amento de raÃ§Ã£o nem o feedback.
  let _protAplicados = [];
  try { _protAplicados = (await _aplicarProtocolosRacao(index, racao, data)) || []; } catch (e) { console.log("Protocolo raÃ§Ã£o:", e); }

  // Mostra mensagem de sucesso e avanÃ§a a data para o dia seguinte (sequÃªncia)
  ...
  // Avisa quais protocolos automÃ¡ticos entraram junto (custo jÃ¡ lanÃ§ado)
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">LanÃ§ar Biometria</h2>
      </div>
      <div class="form-corpo">
        <div class="campo-form">
          <div class="campo-label">
...
async function salvarBiometria(index) {
  ...
  if (dataDuplicada) {
    mostrarErroBio("JÃ¡ existe uma biometria nessa data. Edite ou exclua a existente.");
    return;
  }
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <span class="form-caption">${abreviarViveiro(viveiro.nome)}</span>
        <h2 class="form-titulo">LanÃ§ar Despesca</h2>
      </div>
      <div class="form-corpo">
        <div class="campo-form">
          <div class="campo-label">
        ...
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            <label>PreÃ§o de venda por kg <span style="color:#9ca3af;font-weight:500">(opcional)</span></label>
          </div>
          <div class="campo-input-unidade">
...
async function salvarDespesca(index) {
  ...
  if (!data || !quantidadeKg || !pesoMedio) {
    mostrarErroDespesca("Preencha a data, quantidade e peso mÃ©dio.");
    return;
  }
...
function mostrarHistoricoCultivo(indexSelecionado = "") {
  ...
  if (viveiros.length === 0) {
    area.innerHTML = `
      <div class="resultado-box">
        <p>Nenhum viveiro cadastrado</p>
        <span>Cadastre um viveiro para ver o histÃ³rico.</span>
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">HistÃ³rico</h2>
      </div>
...
function renderizarHistoricoBiometria(index, elementoId, direto) {
  ...
  resultado.innerHTML = `
        ...
        <div class="tabela-historico">
            ...
            ${
              biometrias.length === 0
                ? `<p class="sobrevivencia-texto">Nenhuma biometria lanÃ§ada.</p>`
                : biometrias
                    .map((item, i) => {
                      let crescimento = "-";
                      if (i > 0) {
...
function verCurvaCrescimento(index, direto, pesoAlvo) {
  ...
  // Taxa mÃ©dia de crescimento (g/dia)
  const taxas = [];
  ...
  // Biomassa estimada (usa Ãºltima raÃ§Ã£o + sobrevivÃªncia calculada)
  ...
  // Despescas parciais jÃ¡ realizadas
  ...
  // Monta dados: biometrias reais + pontos semanais interpolados entre elas + projeÃ§Ã£o semanal
  const chartLabels = [];
  const realData = [];
  const projData = [];
  ...
  if (dataPovoamento && gDia > 0 && alvo > pesoAtual) {
    ...
    // Tempo restante a partir de HOJE (nÃ£o da Ãºltima biometria)
    ...
    cardProj = `
      ...
      <p class="proj-obs">Estimativa pelo ganho mÃ©dio de ${formatarNumeroBR(gDia * 7, 2)} g/semana. Quanto mais biometrias, mais precisa.</p>
    `;
  ...
  area.innerHTML = `
    ...
    <div class="grafico-container">
      <h4 class="grafico-titulo">EvoluÃ§Ã£o do crescimento</h4>
      <div class="grafico-legenda">
        <span class="grafico-leg-item"><span class="grafico-leg-dot leg-verde"></span>Biometrias realizadas</span>
        <span class="grafico-leg-item"><span class="grafico-leg-dot leg-laranja"></span>ProjeÃ§Ã£o de crescimento</span>
      </div>
    ...
    ${cardProj}
    <h4 class="bio-hist-titulo">HistÃ³rico de biometrias</h4>
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: ctx2 => {
                ...
                const isProj = ctx2.dataset.label === "ProjeÃ§Ã£o";
...
function renderizarHistoricoRacao(index, elementoId, direto, pagina = 0, direcao = "") {
  ...
  const navProximo = pagina < totalPaginas - 1
    ? `<button class="botao-nav-viveiro" onclick="renderizarHistoricoRacao(${index},'${elementoId}',${direto},${pagina + 1},'proximo')">PrÃ³xima</button>`
    : `<span class="botao-nav-viveiro" style="visibility:hidden">PrÃ³xima</span>`;

  resultado.innerHTML = `
    <h3 class="titulo-secao">RaÃ§Ã£o - ${abreviarViveiro(viveiro.nome)}</h3>

    <div class="tabela-historico">
      <div class="linha-historico-racao cabecalho">
        <span>DIA</span>
        <span class="col-centro">DATA</span>
        <span class="col-centro">RAÃ‡ÃƒO</span>
        <span></span>
      </div>
      ${racoes.length === 0
        ? `<p class="sobrevivencia-texto">Nenhuma raÃ§Ã£o lanÃ§ada.</p>`
    ...
    ${totalPaginas > 1 ? `
      <div class="nav-viveiros">
        ${navAnterior}
        <span class="nav-viveiros-contador">PÃ¡g. ${pagina + 1} / ${totalPaginas}</span>
        ${navProximo}
      </div>
    ` : ""}
  ...
  // AnimaÃ§Ã£o de slide ao trocar pÃ¡gina
  if (direcao) {
...
async function salvarEdicaoBiometria(viveiroIndex, bioIndex, elementoId, direto) {
  ...
  // Impede duas biometrias na mesma data (ignora a prÃ³pria que estÃ¡ sendo editada)
  const dataDuplicada = (viveiros[viveiroIndex].biometrias || [])
    .some((b, idx) => idx !== bioIndex && b.data === novaData);
  if (dataDuplicada) {
    _toastErro("JÃ¡ existe uma biometria nessa data. Edite ou exclua a existente.");
    return;
  }
  ...
  // DELETE + INSERT contorna restriÃ§Ã£o de RLS em UPDATE
  ...
  if (erroIns || !inserido || inserido.length === 0) {
    console.log(erroIns);
    restaurar();
    _toastErro("Erro ao salvar ediÃ§Ã£o. Tente novamente.");
    return;
  }
...
function _montarCustoRacaoVirtual() {
  for (const v of viveiros) {
    ...
    v.custos = (v.custos || []).filter(c => {
      ...
      if (!(c.categoria === "RaÃ§Ã£o" && c.nomeProduto === "RaÃ§Ã£o")) return true;
      if (cicloId && (c.cicloId || null) === cicloId) return false;   // substituÃ­do pelo derivado
    ...
    v.custos.push({
      ...
      nomeProduto: "RaÃ§Ã£o", quantidadeG: der.qtdG, valor: der.valor,
      categoria: "RaÃ§Ã£o", data: der.data,
...
function confirmarExcluirRacao(viveiroIndex, racaoIndex, elementoId, direto, pagina = 0) {
  ...
  row.innerHTML = `
    <div class="confirmar-exclusao-custo" style="grid-column:1/-1">
      <span>Excluir este lanÃ§amento de raÃ§Ã£o?</span>
      <div style="display:flex;gap:8px;margin-top:8px">
...
async function excluirRacao(viveiroIndex, racaoIndex, elementoId, direto, pagina = 0, botao) {
  ...
  if (!racao || !racao.id) {
    _toastErro("Erro: lanÃ§amento sem ID.");
    return;
  }
  ...
  if (error) {
    console.log(error);
    restaurar();
    _toastErro("Erro ao excluir lanÃ§amento.");
    return;
  }

  if (!deletado || deletado.length === 0) {
    restaurar();
    _toastErro("NÃ£o foi possÃ­vel excluir. Verifique sua conexÃ£o ou permissÃ£o.");
    return;
  }
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      ...
      ${povoar ? "" : `<div class="aviso-reinicio">
        ...
        <span>Todo o histÃ³rico de raÃ§Ã£o, biometrias e despescas serÃ¡ <strong>apagado</strong>.</span>
      </div>`}
      <div class="form-corpo">
        <div class="campo-form">
          <div class="campo-label">
        ...
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
            <label>LaboratÃ³rio</label>
          </div>
          <input type="text" id="novoLaboratorio" placeholder="Nome do laboratÃ³rio">
        </div>
        ...
        <button class="botao-salvar ${povoar ? "" : "botao-alerta"}" onclick="salvarNovoCiclo(${index}, '${modo}')">
          ${povoar
            ...
            : `<svg viewBox="0 0 24 24" style="width:18px;height:18px;stroke:white;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15"/></svg> Confirmar reinÃ­cio`}
        </button>
        <div class="separador-ou"><span>ou</span></div>
...
// CORREÃ‡ÃƒO: salvarNovoCiclo agora salva no banco de dados
async function salvarNovoCiclo(index, modo = "reiniciar") {
  ...
  // Povoar (prep cultivo) continua o MESMO ciclo (mantÃ©m o ciclo_id da preparaÃ§Ã£o,
  // para os custos de preparaÃ§Ã£o seguirem no ciclo). Reiniciar comeÃ§a um ciclo novo.
  const povoar = modo === "povoar";
  ...
  viveiros[index].biometrias = [];
  viveiros[index].despescas = [];
  _montarCustoRacaoVirtual(); // zera o custo de raÃ§Ã£o derivado do ciclo novo
...
async function excluirViveiro(index, botao) {
  ...
  if (viveiros.length === 0) {
    mostrarListaViveiros();
    _toastSucesso(`Viveiro "${viveiro.nome}" excluÃ­do.`);
    return;
  }
  mostrarListaViveiros(0, "", `Viveiro "${viveiro.nome}" excluÃ­do com sucesso.`);
}
...
function renderizarHistoricoDespesca(index, elementoId, direto) {
  ...
  resultado.innerHTML = `
        ...
        <div class="tabela-historico">
            ...
            ${
              despescas.length === 0
                ? `<p class="sobrevivencia-texto">Nenhuma despesca lanÃ§ada.</p>`
                : despescas
                    .map((item, i) => `
...
function _toggleDetalheDespesca(index, i) {
  ...
  if (jaAberto) return; // clicou no que jÃ¡ estava aberto sÃ³ fecha
...
function mostrarHistoricoCiclos() {
  esconderMenu();
  const area = document.getElementById("area-gestao");
  const tituloHtml = `<h2 class="titulo-secao">HistÃ³rico de Ciclos</h2>`;
  ...
  if (ciclos.length === 0) {
    area.innerHTML = `
      <div class="form-topo" style="margin-top:8px">
        ...
        <h2 class="form-titulo">HistÃ³rico de Ciclos</h2>
      </div>
  ...
  ` + ciclos.map(item => `
    <div class="ciclo-card">
      ...
      <div class="ciclo-card-infos">
        ...
        <div class="ciclo-info-item">
          <span class="ciclo-info-label">SobrevivÃªncia</span>
      ...
      <div id="confirm-excluir-${item.viveiroIndex}-${item.cicloIndex}" style="display:none;margin-top:10px;padding:10px 12px;background:#fef2f2;border-radius:10px;border:1px solid #fecaca">
        ...
        <p style="margin:0 0 10px;font-size:12px;color:#7f1d1d">Esta aÃ§Ã£o nÃ£o pode ser desfeita.</p>
...
async function excluirCiclo(viveiroIndex, cicloIndex, botao) {
  ...
  if (!deletado || deletado.length === 0) {
    restaurar();
    const div = document.getElementById(`confirm-excluir-${viveiroIndex}-${cicloIndex}`);
    if (div) div.innerHTML = `<p style="color:#dc2626;font-size:13px;margin:0">NÃ£o foi possÃ­vel excluir. Verifique as permissÃµes no Supabase (RLS da tabela ciclos).</p>`;
    return;
  }
...
function _statusBoleto(dataCompra, prazoDias) {
  ...
  if (diff < 0) return { tipo: "vencido", dias: Math.abs(diff), diff, label: `Vencido hÃ¡ ${Math.abs(diff)} dias`, dataFmt };
    ...
    <div class="cfg-wrap">
      ...
      <div class="cfg-lista">
        <button class="cfg-item" onclick="abrirFinanceiro()">
          ...
          <div class="cfg-item-texto"><span class="cfg-item-titulo">RelatÃ³rio financeiro</span><span class="cfg-item-sub">Consulte os custos por viveiro ou geral</span></div>
          <svg class="cfg-item-chevron" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
        </button>
...
const _PLANOS_APP = [
  { key: "basico",        nome: "BÃ¡sico",        viveiros: "2 a 5 viveiros", mensal: 50,  anual: 500 },
  { key: "intermediario", nome: "IntermediÃ¡rio", viveiros: "atÃ© 10 viveiros", mensal: 90,  anual: 900 },
  { key: "avancado",      nome: "AvanÃ§ado",      viveiros: "atÃ© 20 viveiros", mensal: 160, anual: 1600 },
...
// Converte "YYYY-MM-DD" (ou timestamp) numa Data local Ã  meia-noite, ou null.
...
// Pagamento pendente ainda dentro da carÃªncia? (nÃ£o pune enquanto o
// vencimento nÃ£o passou de _DIAS_CARENCIA; sem vencimento conhecido, nÃ£o pune)
...
// Limite de viveiros do plano vigente. GrÃ¡tis / plano pago bloqueado = 1.
...
// A conta estÃ¡ em modo somente leitura?
// SÃ³ trava plano PAGO cujo pagamento parou (pendente/cancelado) e que jÃ¡
// passou da carÃªncia. Nunca trava quem estÃ¡ no grÃ¡tis nem quem estÃ¡ em dia.
function _contaBloqueada() {
  const a = assinatura;
  if (!a) return false;                                // sem assinatura nÃ£o trava
  if (!a.plano || a.plano === "gratis") return false;  // grÃ¡tis nÃ£o trava
  if (a.status === "ativo") return false;              // pagamento em dia
  if (viveiros.length <= 1) return false;              // dentro do grÃ¡tis (1 viveiro)
  if (a.status === "cancelado") return true;           // assinatura cancelada sÃ³ leitura
  return !_dentroDaCarencia(a);                        // pendente: trava sÃ³ apÃ³s a carÃªncia
}

// Guarda de escrita GLOBAL: retorna true (e avisa) quando a conta inteira
// estÃ¡ travada (plano pago vencido). Usado em funÃ§Ãµes que nÃ£o sÃ£o de um
// viveiro especÃ­fico (catÃ¡logos, boletos, custos fixos, produtos).
function _bloqueioEdicao() {
  if (_contaBloqueada()) {
    _toastErro("Conta em modo somente leitura. Regularize o pagamento para lanÃ§ar ou editar.");
    return true;
  }
  return false;
}

// Um viveiro (pela POSIÃ‡ÃƒO na lista ordenada) estÃ¡ fora do limite do plano?
// Ex.: no grÃ¡tis (limite 1), sÃ³ o 1Âº viveiro fica liberado; do 2Âº em diante
// fica em modo leitura, mesmo jÃ¡ estando cadastrado.
...
// Guarda de escrita POR VIVEIRO: bloqueia se a conta estÃ¡ travada OU se este
// viveiro especÃ­fico estÃ¡ alÃ©m do limite do plano.
...
// Banner de aviso no menu quando a conta estÃ¡ em somente leitura.
...
function abrirAssinatura() {
  ...
  // Bloco de status unificado (plano atual + info do grÃ¡tis, sem repetir aviso)
  let statusL1, statusL2, statusCls;
  if (a.status === "ativo" && !ehGratis) {
    statusL1 = `Plano atual: <b>${_planoLabel(a.plano)}</b>`;
    statusL2 = a.ciclo ? `CobranÃ§a ${a.ciclo} Â· ativa` : "Assinatura ativa";
    statusCls = "ok";
    ...
    statusL1 = `Plano atual: <b>GrÃ¡tis</b>`;
    statusL2 = "1 viveiro ativo Â· sem cobranÃ§a";
    statusCls = "free";
  }
  ...
  const cards = _PLANOS_APP.map(p => {
    ...
    if (ciclo === "anual") {
      ...
      precoBloco = `
        ...
        <div class="plano-preco-sub">equivale a R$ ${formatarNumeroBR(equiv, 2)}/mÃªs</div>
      ...
      <div class="plano-card${atual ? " plano-card-atual" : ""}">
        <div class="plano-card-corpo">
          ...
          <ul class="plano-recursos">
            <li>GestÃ£o completa dos ciclos</li>
            <li>Financeiro e relatÃ³rios</li>
            <li>HistÃ³rico de cultivo</li>
          </ul>
        </div>
...
// Porta de entrada legada: qualquer botÃ£o antigo "Assinar" cai no WhatsApp.
...
function abrirSimularVenda() {
  ...
  if (!ativos.length) {
    area.innerHTML = `
      ...
      <div class="cfg-wrap">
        <p class="sobrevivencia-texto" style="margin:18px 0">Nenhum viveiro em cultivo ativo para simular.<br><small>Cadastre um ciclo e lance raÃ§Ã£o/biometria primeiro.</small></p>
        <button class="botao-voltar-form" onclick="voltarMenuGestao()">Voltar</button>
      </div>`;
    return;
  }
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      ...
      <div class="campo-form">
        <div class="campo-label">
          <svg class="campo-icone" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
          <label>PreÃ§o de venda (R$/kg)</label>
        </div>
...
function _simVendaCalcular() {
  ...
  if (!dados || !dados.biomassaAtual) {
    resultado.innerHTML = `<div class="sim-aviso">Ainda nÃ£o dÃ¡ pra estimar a biomassa deste viveiro. Ã‰ preciso ter pelo menos uma <b>biometria</b> e <b>raÃ§Ã£o</b> lanÃ§ada.</div>`;
    return;
  }

  const biomassaAtual = dados.biomassaAtual;              // o que ainda estÃ¡ no viveiro (a vender)
  const despKgTotal = dados.despKgTotal;                  // o que jÃ¡ foi despescado
  const biomassaTotal = biomassaAtual + despKgTotal;      // produÃ§Ã£o total (base do custo/kg e lucro/kg)
  ...
  const cabecalho = `
    <div class="sim-biomassa">Biomassa estimada: <b>${formatarNumeroBR(biomassaTotal, 0)} kg</b>${despKgTotal > 0 ? ` <small>(${formatarNumeroBR(biomassaAtual, 0)} kg em pÃ© + ${formatarNumeroBR(despKgTotal, 0)} kg despescado)</small>` : ""}${dados.pesoUltimaBio ? ` Â· peso mÃ©dio ${formatarNumeroBR(dados.pesoUltimaBio, 1)} g` : ""}</div>`;
  ...
  if (preco <= 0) {
    resultado.innerHTML = cabecalho + `
      ...
      <div class="sim-hint">Digite o preÃ§o por kg acima para ver faturamento e lucro.</div>`;
    return;
  }

  // Receita das despescas JÃ VENDIDAS: usa o preÃ§o REAL de cada despesca.
  // Se alguma despesca nÃ£o tiver preÃ§o salvo, cai no preÃ§o simulado (aproximaÃ§Ã£o).
  let kgSemPreco = 0;
  ...
  resultado.innerHTML = cabecalho + `
    <div class="sim-cards">
      ...
      <div class="sim-card ${ok ? "sim-ok" : "sim-neg"}"><small>Lucro por mÃªs</small><strong>R$ ${formatarNumeroBR(lucroMes, 2)}</strong></div>
    </div>
    <div class="sim-hint">Faturamento = biomassa em pÃ© Ã— preÃ§o simulado + receita real das despescas jÃ¡ vendidas. Lucro/mÃªs = lucro Ã· ${formatarNumeroBR(meses, 1)} ${meses >= 2 ? "meses" : "mÃªs"}.</div>
    ${avisoSemPreco}`;
}
...
function abrirCustosFixos() {
  ...
  const cards = custosFixos.length === 0
    ? `<p class="sobrevivencia-texto" style="margin:18px 0">Nenhum custo fixo cadastrado ainda.<br><small>Cadastre mÃ£o de obra, energia e outros custos mensais para rateÃ¡-los automaticamente entre os viveiros.</small></p>`
    : custosFixos.map((c, i) => `
        <div class="cf-card${c.ativo ? "" : " cf-card-off"}">
          ...
          <div class="cf-card-info">
            ...
            <span class="cf-card-cat">${_custoFixoCatLabel(c.categoria)}${c.dataInicio ? " Â· desde " + formatarData(c.dataInicio) : ""}${c.ativo ? "" : " Â· inativo"}</span>
          </div>
          <div class="cf-card-valor">R$ ${formatarNumeroBR(c.valorMensal, 2)}<small>/mÃªs</small></div>
  ...
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      <div class="cf-explica">
        ...
        <span>Estes custos sÃ£o <strong>rateados por dia</strong> entre os viveiros ativos (em preparaÃ§Ã£o ou em cultivo) e entram automaticamente no custo de cada ciclo.</span>
      </div>
      <div class="cf-resumo">
...
async function salvarCustoFixo(index) {
  ...
  if (!valorMensal || valorMensal <= 0) { mostrarErro("Informe um valor mensal vÃ¡lido."); return; }
...
async function excluirCustoFixo(index, botao) {
  ...
  _toastSucesso("Custo fixo excluÃ­do.");
  abrirCustosFixos();
}
...
function abrirBoletos(filtro) {
  ...
  // Total A PAGAR = o que ainda falta (jÃ¡ abate os pagamentos parciais)
  ...
  // Se o fornecedor selecionado nÃ£o existe mais, limpa o filtro
  ...
  area.innerHTML = `
    <div class="fin-topo-acoes">
      ...
      <div class="bt-topo-btns">
        <button class="fin-novo-btn fin-novo-btn-sec" onclick="imprimirBoletos()">ğŸ–¨ï¸ Imprimir</button>
...
function _filtrarBoletosBusca(termo) {
  ...
  document.querySelectorAll(".bt-lista .bt-card").forEach(el => {
    ...
    if (!ok) return;
    vis++;
    // Recalcula o total sÃ³ com os cards VISÃVEIS (coerÃªncia busca Ã— total)
...
function imprimirBoletos() {
  ...
  // TOTAL da impressÃ£o: pagos total pago; demais o que ainda falta pagar
  ...
  <h1>${titulo}</h1>
  <p class="sub">Emitido em ${hoje} Â· ${filtrados.length} boleto${filtrados.length > 1 ? "s" : ""}</p>
  <table>
    <thead><tr><th>Nome</th><th>Fornecedor</th><th style="text-align:center">Vencimento</th><th style="text-align:center">SituaÃ§Ã£o</th><th style="text-align:right">Valor</th></tr></thead>
    <tbody>
      ${linhas}
...
// Quanto ainda falta pagar de um boleto (0 se nÃ£o tiver valor total definido)
function _boletoRestante(b) {
  if (!b.valor || b.valor <= 0) return 0;
  return Math.max(0, b.valor - (b.valorPago || 0));
}

// Barra de progresso "pago X de Y Â· falta Z" (sÃ³ quando hÃ¡ valor total)
...
async function salvarPagamentoParcial(index, botao) {
  ...
  if (isNaN(valor) || valor <= 0) { _toastErro("Informe um valor de pagamento vÃ¡lido."); if (inp) inp.focus(); return; }
  const rest = _boletoRestante(b);
...
async function desfazerUltimoPagamento(index) {
  ...
  // Se nÃ£o havia parcelas registradas (quitaÃ§Ã£o direta), apenas reabre o boleto
...
function abrirFormBoleto(index) {
  ...
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      <div class="fin-secao-titulo">InformaÃ§Ãµes do boleto</div>
      <div class="campo-form">
        <div class="campo-label">
        ...
        <input type="text" id="boleto-nome" placeholder="Ex: RaÃ§Ã£o ABC" value="${b ? b.nome : ""}">
      </div>
      <div class="campo-form">
        <div class="campo-label">
          <svg class="campo-icone" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        ...
        <input type="text" id="boleto-fornecedor" placeholder="Ex: Loja do JoÃ£o" value="${b ? b.fornecedor : ""}">
      </div>
      <div class="campo-form">
        <div class="campo-label">
          <svg class="campo-icone" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
...
async function marcarBoletoPago(index, voltarDetalhe) {
  ...
  // Se tem valor total, quita o restante e registra esse pagamento no histÃ³rico
...
async function desmarcarBoletoPago(index, voltarDetalhe) {
  ...
  const usuario = await pegarUsuarioLogado();
  if (!usuario) return;
  // Reabre e zera os pagamentos (aÃ§Ã£o rÃ¡pida de "desfazer" a partir da lista)
  const { error } = await supabaseClient.from("boletos")
...
function abrirFinanceiro() {
  ...
  area.innerHTML = `
    <div class="fin-topo-acoes">
      <h3 class="titulo-secao" style="margin:0">RelatÃ³rio financeiro</h3>
      <button class="fin-novo-btn" onclick="imprimirRelatorioFinanceiro()">ğŸ–¨ï¸ Imprimir</button>
    </div>
    <div class="cfg-wrap">
      <div class="campo-form">
        <div class="campo-label">
...
function _finTipoLabel(c) {
  if (c.tipo === "fixo") return "Rateio automÃ¡tico";
...
function _finItensRateioFixo(alvos) {
  ...
  for (const v of alvos) {
    ...
    for (const cf of custosFixos) {
      ...
      // Janela = max(data_inicio do custo, inÃ­cio do viveiro) .. hoje,
      // interceptada com o perÃ­odo do filtro financeiro.
      let ini = vIni;
      ...
      if (ini > fim) continue;
      // Acumula a parcela diÃ¡ria deste custo (funÃ§Ã£o-base compartilhada, dias reais do mÃªs)
...
function _finColetarCustos() {
  ...
  // Injeta os itens virtuais de rateio dos custos fixos (nÃ£o editÃ¡veis pela lista)
...
function mostrarCustosFinanceiro() {
  ...
  if (custos.length === 0) {
    resultado.innerHTML = `<p class="sobrevivencia-texto" style="margin:16px 0">Nenhum custo lanÃ§ado${porViveiro ? " para este viveiro" : ""} no perÃ­odo.<br><small>Ajuste os filtros ou lance custos dentro de cada viveiro.</small></p>`;
    return;
  }
...
function _finRenderDetalhado(resultado, custos, total, porViveiro) {
  ...
  // ConsolidaÃ§Ã£o por categoria (visÃ£o "Todos os viveiros")
  const grupos = !porViveiro ? _finGruposCategoria(custos) : null;
  const maiorCat = grupos && grupos.length ? grupos[0] : null;

  // 4Âº card: por viveiro mostra o maior lanÃ§amento; consolidado, a maior categoria
  ...
  const listaBlocoHtml = porViveiro
    ? `<div class="fin-lista-head">
        <span>LanÃ§amentos de custos</span>
        <select class="fin-ordenar" onchange="_finOrdenacao=this.value;_finPagina=0;mostrarCustosFinanceiro()">
          ...
          <option value="descricao" ${ord === "descricao" ? "selected" : ""}>DescriÃ§Ã£o</option>
        </select>
      </div>
      <div class="fin-lista">
        ${pagina.map(c => `
      ...
      ${totalPag > 1 ? `
        <div class="fin-paginacao">
          ...
          <span>PÃ¡g. ${_finPagina + 1} / ${totalPag}</span>
          <button ${_finPagina >= totalPag - 1 ? "disabled" : ""} onclick="_finPagina++;mostrarCustosFinanceiro()">PrÃ³xima</button>
        </div>` : ""}`
    : `<div class="fin-lista-head">
      ...
      <div class="fin-lista">
        ${grupos.map(g => `
          <div class="fin-linha fin-linha-cat">
            <span class="fin-linha-desc">${g.nome}<small>${g.qtd} lanÃ§amento${g.qtd > 1 ? "s" : ""} Â· ${g.viveiros.size} viveiro${g.viveiros.size > 1 ? "s" : ""}</small></span>
  ...
  resultado.innerHTML = `
    <div class="fin-cards">
      ...
      <div class="fin-card">
        <div class="fin-card-top"><svg viewBox="0 0 24 24"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg><span>LanÃ§amentos</span></div>
        <strong>${custos.length}</strong>
        <small>Itens registrados</small>
      </div>
      <div class="fin-card">
        <div class="fin-card-top"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg><span>Custo mÃ©dio / dia</span></div>
        <strong>R$ ${formatarNumeroBR(mediaDia, 2)}</strong>
        <small>No perÃ­odo</small>
      </div>
      ${cardMaiorHtml}
    </div>
    ...
    <div class="fin-total-geral">
      ...
      <div class="fin-total-geral-txt">
        <span>Total geral no perÃ­odo</span>
        <strong>R$ ${formatarNumeroBR(total, 2)}</strong>
      </div>
    </div>
  `;
...
function imprimirRelatorioFinanceiro() {
  const { custos, porViveiro } = _finColetarCustos();
  if (!custos.length) { _toastErro("Nenhum custo no perÃ­odo para imprimir."); return; }
  const total = custos.reduce((s, c) => s + Number(c.valor), 0);
  const periodoTxt = (_finPeriodoIni || _finPeriodoFim)
    ? `${_finPeriodoIni ? formatarData(_finPeriodoIni) : "inÃ­cio"} atÃ© ${_finPeriodoFim ? formatarData(_finPeriodoFim) : "hoje"}`
    : "Todo o perÃ­odo";
  let cabecalho, linhas, subtitulo;
  if (porViveiro) {
    // Um viveiro: detalhe lanÃ§amento a lanÃ§amento
    subtitulo = custos[0].viveiroNome || "";
    const ordenados = [...custos].sort((a, b) => b.data.localeCompare(a.data));
    cabecalho = `<tr><th>Data</th><th>Viveiro</th><th>DescriÃ§Ã£o</th><th>Valor</th></tr>`;
    ...
    cabecalho = `<tr><th>Categoria</th><th style="text-align:center">LanÃ§amentos</th><th style="text-align:center">Viveiros</th><th>Valor</th></tr>`;
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>RelatÃ³rio financeiro</title>
    ...
    <body><h1>RelatÃ³rio financeiro</h1><p class="sub">${subtitulo}</p><p>PerÃ­odo: ${periodoTxt}</p>
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        <div style="text-align:center;padding:24px 16px;background:#f8fafc;border-radius:14px;border:1px solid #e5e7eb;margin-bottom:12px">
          <p style="font-size:32px;margin:0 0 8px">ğŸš§</p>
          <p style="font-size:14px;font-weight:700;color:#374151;margin:0 0 4px">Em desenvolvimento</p>
          <p style="font-size:13px;color:#9ca3af;margin:0">Em breve vocÃª poderÃ¡ controlar o estoque dos seus insumos.</p>
        </div>
        <button class="botao-voltar-form" onclick="voltarMenuGestao()">Voltar</button>
      </div>
    </div>
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        ...
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            <label>PreÃ§o da despesca final (R$/kg)</label>
          </div>
          <div class="campo-input-unidade">
...
async function salvarEncerramentoCiclo(index) {
  ...
  if (!dataEncerramento || !producaoFinal || !pesoFinal) {
    mostrarErroEncerrar("Preencha data de encerramento, produÃ§Ã£o final e peso mÃ©dio final.");
    return;
  }
  ...
  // Protege contra tamanho/total povoado ausentes (evita Infinity/NaN no relatÃ³rio)
  ...
  // SobrevivÃªncia correta: cada despesca parcial conta com o SEU peso mÃ©dio,
  // nÃ£o com o peso final. (Ex.: parcial 200kg@10g = 20.000 animais.)
  ...
  const cicloBanco = {
    ...
    // Persiste o histÃ³rico do ciclo para os relatÃ³rios continuarem completos
  ...
  // Apagar todos os lanÃ§amentos do ciclo encerrado no banco
  ...
  // banco (no dia a dia ele Ã© derivado dos lanÃ§amentos; ao encerrar, os
  // lanÃ§amentos sÃ£o apagados, entÃ£o o valor precisa ficar gravado p/ histÃ³rico)
  try {
    ...
    if (der && viveiro.cicloId) {
      ...
      if (iniCiclo) {
        await supabaseClient.from("custos").delete()
          .eq("viveiro_id", viveiro.id).eq("user_id", usuario.id)
          .eq("categoria", "RaÃ§Ã£o").eq("nome_produto", "RaÃ§Ã£o")
          .is("ciclo_id", null)
      ...
      const { data: salvoSnap } = await supabaseClient.from("custos").insert([{
        ...
        nome_produto: "RaÃ§Ã£o", quantidade_g: der.qtdG, valor: der.valor,
        categoria: "RaÃ§Ã£o", data: der.data, ciclo_id: viveiro.cicloId,
      }]).select();
      if (salvoSnap) {
        viveiro.custos = (viveiro.custos || []).filter(c => !(c.categoria === "RaÃ§Ã£o" && c.nomeProduto === "RaÃ§Ã£o" && (c.cicloId || null) === viveiro.cicloId));
        viveiro.custos.push({ id: salvoSnap[0].id, tipo: "produto", produtoId: null, nomeProduto: "RaÃ§Ã£o", quantidadeG: der.qtdG, valor: der.valor, categoria: "RaÃ§Ã£o", data: der.data, observacao: null, cicloId: viveiro.cicloId });
      }
    }
  } catch (e) { console.log("Congelar custo de raÃ§Ã£o:", e); }
  ...
  // Zera o ciclo e volta o viveiro para "Em preparaÃ§Ã£o" (conta desde o encerramento)
  ...
  _montarCustoRacaoVirtual(); // ciclo novo comeÃ§a sem custo de raÃ§Ã£o derivado
    ...
    <div class="form-lancamento">
      ...
      <div class="prep-status">
        ...
        <div class="prep-status-txt">
          <span class="prep-status-lbl">Em preparaÃ§Ã£o</span>
      ...
      <button class="botao-voltar-form" onclick="abrirLancarCusto(${index})" style="margin-top:8px">
        ...
        LanÃ§ar custo de preparaÃ§Ã£o
      </button>
      ` : `
      <div class="viveiro-sem-ciclo-msg">
        <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        <span>Nenhum ciclo ativo. Inicie um novo ciclo para comeÃ§ar os lanÃ§amentos.</span>
      </div>
      ...
      <button class="botao-voltar-form" onclick="mostrarRelatorioCiclo(${index}, viveiros[${index}].ciclosFinalizados[viveiros[${index}].ciclosFinalizados.length - 1], 'viveiro')" style="margin-top:8px">
        ğŸ“‹ Ver relatÃ³rio do Ãºltimo ciclo
      </button>
      ` : ""}
...
function mostrarRelatorioCiclo(index, ciclo, origem = "historico") {
  ...
  const precoFinal = Number(ciclo.precoVenda) || 0; // preÃ§o da despesca final
  ...
  area.innerHTML = `
    <div class="rc2-report">
      ...
      <div class="rc2-head">
        <h2 class="rc2-titulo">RELATÃ“RIO DE CICLO</h2>
        <div class="rc2-viveiro">${ciclo.nomeViveiro}</div>
        <div class="rc2-periodo">${formatarData(ciclo.dataPovoamento)} a ${formatarData(ciclo.dataEncerramento)} Â· ${ciclo.diasCultivo} dias</div>
      </div>

      <div class="rc2-sec-tit">InformaÃ§Ãµes gerais</div>
      <div class="rc2-grid">
      ...
      <div class="rc2-band">
        ...
        <div><b>${formatarNumeroBR(ciclo.sobrevivencia, 1)}%</b><span>SobrevivÃªncia</span></div>
        <div><b>${formatarNumeroBR(ciclo.fca, 2)}</b><span>FCA</span></div>
      </div>

      <div class="rc2-sec-tit">Resumo operacional</div>
      <div class="rc2-lines">
        <div class="rc2-line"><span>RaÃ§Ã£o consumida</span><b>${formatarNumeroBR(ciclo.racaoConsumida, 1)} kg</b></div>
      ...
      <div class="rc2-sec-tit">EvoluÃ§Ã£o do cultivo</div>
      <div class="rc-graficos">
        <div class="rc-graf-box"><h5>Peso mÃ©dio (g)</h5><div class="rc-graf-canvas"><canvas id="rcPeso"></canvas></div></div>
        <div class="rc-graf-box"><h5>Consumo acumulado de raÃ§Ã£o (kg)</h5><div class="rc-graf-canvas"><canvas id="rcRacao"></canvas></div></div>
      </div>
      ` : ""}
...
function _seriesCiclo(ciclo) {
  ...
  bios.forEach((b, i) => {
    ...
    const bm = restante * b.gramatura / 1000; // biomassa em pÃ© estimada na data
...
function _renderGraficosCiclo(serie) {
  ...
  // RelatÃ³rio final: apenas Peso mÃ©dio e Consumo acumulado de raÃ§Ã£o (sem
  // grÃ¡ficos estimados de FCA/biomassa).
...
function gerarRelatorioImpressao() {
  ...
  if (!ciclo) { _toastErro("RelatÃ³rio indisponÃ­vel."); return; }

  // PreÃ§o automÃ¡tico: usa o preÃ§o da despesca final (encerramento); cada despesca
  ...
  // preÃ§o geral do relatÃ³rio. A despesca final usa o preÃ§o geral (do encerramento).
  ...
  const indicadores = [
    { lbl: "Peso mÃ©dio final", val: fmt(ciclo.pesoFinal, 1) + " g" },
    ...
    { lbl: "SobrevivÃªncia", val: fmt(ciclo.sobrevivencia, 1) + " %" },
    { lbl: "RaÃ§Ã£o consumida", val: fmt(ciclo.racaoConsumida, 1) + " kg" },
    { lbl: "Custo total", val: "R$ " + fmt(custoTotal, 2) },
    { lbl: "Custo por kg", val: "R$ " + fmt(custoPorKg, 2) },
    { lbl: "Receita bruta", val: rs(receitaBruta) },
    { lbl: "Lucro lÃ­quido", val: rs(lucroLiquido) },
  ];
  ...
  const _fr = [];
  _fr.push(`Com base nos dados registrados, o ciclo teve ${_dc} ${_dc === 1 ? "dia" : "dias"} de cultivo, com sobrevivÃªncia estimada de ${fmt(_sob, 1)}% e produÃ§Ã£o total de ${fmt(producaoTotal, 1)} kg (${fmt(ciclo.produtividade, 1)} kg/ha).`);
  if ((Number(ciclo.racaoConsumida) || 0) > 0) _fr.push(`Foram consumidos ${fmt(ciclo.racaoConsumida, 1)} kg de raÃ§Ã£o, com FCA final de ${fmt(_fca, 2)}.`);
  else _fr.push(`NÃ£o houve consumo de raÃ§Ã£o registrado no perÃ­odo.`);
  ...
  if (temPreco) _fr.push(lucroLiquido >= 0
    ? `O resultado financeiro foi positivo, com lucro lÃ­quido de R$ ${fmt(lucroLiquido, 2)}.`
    : `O resultado financeiro foi negativo, com prejuÃ­zo de R$ ${fmt(Math.abs(lucroLiquido), 2)}.`);
  _fr.push(`O resultado deve ser interpretado considerando densidade, salinidade, manejo, qualidade da Ã¡gua, estratÃ©gia alimentar e condiÃ§Ãµes locais.`);
  const conclusaoTecnica = _fr.join(" ");

  // Alertas tÃ©cnicos e de dados incompletos (critÃ©rios explÃ­citos)
  const _alertas = [];
  if (_sob > 100) _alertas.push("A sobrevivÃªncia estimada ficou acima de 100%, indicando possÃ­vel inconsistÃªncia nos registros de povoamento, pesos ou quantidades.");
  ...
  const alertasHtml = _alertas.length
    ? `<div class="rc-alertas-dados"><strong>Pontos de atenÃ§Ã£o</strong><ul>${_alertas.map(a => `<li>${a}</li>`).join("")}</ul></div>`
    : "";
  ...
  const execCards = [
    { ico: _ico.prod, lbl: "ProduÃ§Ã£o final", val: fmt(producaoTotal, 1) + " kg", sub: fmt(ciclo.produtividade, 1) + " kg/ha", cls: "" },
    { ico: _ico.sobr, lbl: "SobrevivÃªncia", val: fmt(ciclo.sobrevivencia, 1) + " %", sub: "", cls: "" },
  ...
  <h2 class="sec">8. AlimentaÃ§Ã£o</h2>
  <div class="grid info6">
    <div class="cel"><small>RaÃ§Ã£o consumida</small><b>${fmt(racaoKg, 1)} kg</b></div>
    <div class="cel"><small>Custo da raÃ§Ã£o</small><b>R$ ${fmt(custoRacao, 2)}</b></div>
    <div class="cel"><small>% do custo total</small><b>${custoTotal > 0 ? fmt(custoRacao / custoTotal * 100, 1) : "0"}%</b></div>
    <div class="cel"><small>Custo raÃ§Ã£o / kg produzido</small><b>${producaoTotal > 0 ? "R$ " + fmt(custoRacao / producaoTotal, 2) : "-"}</b></div>
    <div class="cel"><small>PreÃ§o mÃ©dio da raÃ§Ã£o</small><b>${racaoKg > 0 ? "R$ " + fmt(custoRacao / racaoKg, 2) + "/kg" : "-"}</b></div>
  ...
  despescasArr.forEach(d => _eventos.push({ d: d.data, t: "Despesca", i: `${fmt(Number(d.quantidadeKg) || 0, 1)} kg Â· ${fmt(Number(d.pesoMedio) || 0, 1)} g` }));
  if (ciclo.dataEncerramento) _eventos.push({ d: ciclo.dataEncerramento, t: "Encerramento", i: `ProduÃ§Ã£o ${fmt(producaoTotal, 1)} kg` });
  ...
  <h2 class="sec">10. ComparaÃ§Ã£o com ciclos anteriores</h2>
  <table><thead><tr><th>Encerramento</th><th class="num">Dias</th><th class="num">ProduÃ§Ã£o (kg)</th><th class="num">Produtividade (kg/ha)</th><th class="num">Peso mÃ©dio (g)</th></tr></thead>
  <tbody>${_hist.map(c => {
...
<body><div class="doc">
  ...
  <div class="cab">
    <div class="cab-marca"><div class="cab-logo">WA<br>AQUA</div><div><b>WA Aqua GestÃ£o</b><br><small>TECNOLOGIA PARA AQUICULTURA</small></div></div>
    <div class="cab-centro"><h1>RELATÃ“RIO FINAL DO CICLO</h1><span class="viv">${(ciclo.nomeViveiro || "").toUpperCase()}</span></div>
    <div class="cab-periodo"><small>PERÃODO DO CICLO</small><b>${formatarData(ciclo.dataPovoamento)} a ${formatarData(ciclo.dataEncerramento)}</b>${ciclo.diasCultivo} dias de cultivo</div>
  </div>
  ...
  <h2 class="sec">1. InformaÃ§Ãµes gerais</h2>
  <div class="grid info6">
    ...
    <div class="cel"><small>LaboratÃ³rio</small><b>${ciclo.laboratorio || "-"}</b></div>
    <div class="cel"><small>Ãrea do viveiro</small><b>${fmt(tamanhoNum, 1)} ha</b></div>
  ...
  <div class="grupos3">
    <div class="grupo">
      <h5>ProduÃ§Ã£o</h5>
      <div class="grupo-row"><span>Peso mÃ©dio final</span><b>${fmt(ciclo.pesoFinal, 1)} g</b></div>
    ...
    <div class="grupo">
      ...
      <div class="grupo-row"><span>SobrevivÃªncia</span><b>${fmt(ciclo.sobrevivencia, 1)} %</b></div>
      <div class="grupo-row"><span>RaÃ§Ã£o consumida</span><b>${fmt(ciclo.racaoConsumida, 1)} kg</b></div>
    </div>
    <div class="grupo">
      <h5>Financeiro</h5>
      ...
      <div class="grupo-row"><span>Lucro lÃ­quido</span><b>${rs(lucroLiquido)}</b></div>
    </div>
  </div>

  <div class="duas" style="margin-top:18px">
    <div>
      <h2 class="sec" style="margin-top:0">3. EvoluÃ§Ã£o do cultivo</h2>
      <div class="charts">
        <div class="chart-box"><h4>EvoluÃ§Ã£o do peso mÃ©dio (g)</h4><canvas id="cPeso"></canvas></div>
        <div class="chart-box"><h4>Consumo acumulado de raÃ§Ã£o (kg)</h4><canvas id="cRacao"></canvas></div>
      </div>
    </div>
    <div>
      <h2 class="sec" style="margin-top:0">4. DistribuiÃ§Ã£o dos custos</h2>
      ${distLista.length ? `<div class="rosca-wrap"><div class="rosca-canvas"><canvas id="cDist"></canvas><div class="rosca-centro"><small>TOTAL</small><b>R$ ${fmt(custoTotal,2)}</b></div></div><div class="leg">${legendaDist}</div></div>` : `<p style="color:#9ca3af;font-size:11px">Nenhum custo lanÃ§ado neste ciclo.</p>`}
      ...
      <div class="fin-row destaque"><span>Lucro lÃ­quido</span><b>${rs(lucroLiquido)}</b></div>
  ...
  <table><thead><tr><th>Data</th><th class="num">Dias</th><th class="num">Peso mÃ©dio (g)</th><th class="num">Crescimento (g)</th><th>ObservaÃ§Ãµes</th></tr></thead>
  ...
  <table><thead><tr><th>Data</th><th>Tipo</th><th class="num">Quantidade (kg)</th><th class="num">Peso mÃ©dio (g)</th><th class="num">PreÃ§o/kg</th><th class="num">Receita (R$)</th></tr></thead>
  ...
  <div class="assin"><div class="linha"></div><small>ResponsÃ¡vel tÃ©cnico</small></div>

  <div class="rodape">
    <span>RelatÃ³rio gerado automaticamente pelo WA Aqua GestÃ£o.</span>
    <span style="text-align:right">EmissÃ£o: ${dataEmissao} Ã s ${horaEmissao}</span>
  </div>

  <div class="no-print" style="text-align:center;margin-top:18px"><button class="btn-print" onclick="window.print()">ğŸ–¨ï¸ Imprimir / Salvar PDF</button></div>
</div>
...
<\/script>
  ...
  if (!win) { _toastErro("Permita pop-ups para gerar o relatÃ³rio."); return; }
...
// Soma mensal dos custos que jÃ¡ valiam numa data (respeita a data de inÃ­cio de cada um)
function _custoFixoMensalNaData(ymd) {
  return custosFixos.reduce((s, c) => {
    if (c.ativo === false) return s;
    if (c.dataInicio && ymd && c.dataInicio > ymd) return s; // ainda nÃ£o comeÃ§ou nessa data
    return s + (Number(c.valorMensal) || 0);
  }, 0);
}
// Dias reais do mÃªs civil de uma data (28/29/30/31)
...
// Quantos viveiros estavam ativos (preparaÃ§Ã£o ou cultivo) numa data (YYYY-MM-DD)
function _viveirosAtivosNaData(ymd, hojeYmd) {
  let n = 0;
  for (const v of viveiros) {
    // Ciclo atual em andamento (preparaÃ§Ã£o ou cultivo)
    const ini = v.dataPreparacao || v.dataPovoamento;
    if (ini && ini <= ymd && ymd <= hojeYmd) { n++; continue; }
    // Ciclos jÃ¡ finalizados desse viveiro (perÃ­odos passados)
...
// Custo fixo rateado acumulado para um viveiro no perÃ­odo [iniYmd, fimYmd] (inclusive)
function _custoFixoRateado(iniYmd, fimYmd) {
  ...
  while (cur <= fimYmd && guard < 5000) {
    const mensalNoDia = _custoFixoMensalNaData(cur); // soma dos custos que jÃ¡ valiam nesse dia
    if (mensalNoDia > 0) {
...
// RÃ³tulo amigÃ¡vel da categoria
function _custoFixoCatLabel(cat) {
  return ({ mao_de_obra: "MÃ£o de obra", energia: "Energia", aluguel: "Aluguel", agua: "Ãgua", manutencao: "ManutenÃ§Ã£o", outro: "Outro" })[cat] || "Outro";
}

// Custos MANUAIS (reais) que pertencem a um ciclo. Casam pelo ciclo_id quando
// ambos tÃªm; senÃ£o (legados sem ciclo_id, criados antes do backfill) usam a
// janela de datas [ini, fim]. Corrige a regressÃ£o em que o backfill deu ciclo_id
...
function _custosManuaisDoCiclo(custos, cicloId, iniYmd, fimYmd) {
  ...
  return (custos || []).filter(c => {
    if (cicloId && c.cicloId) return c.cicloId === cicloId; // ambos tÃªm id casa exato
...
async function _lancarCustoAuto(index, produto, quantidadeG, data, obs) {
  ...
  // Nunca repete o mesmo lanÃ§amento automÃ¡tico (mesmo produto + data)
  const jaTem = (viveiros[index].custos || []).some(c =>
    c.data === data && c.produtoId === produto.id && (c.observacao || "").startsWith("AutomÃ¡tico"));
  ...
  const { data: salvo, error } = await supabaseClient.from("custos").insert([{
    ...
    valor, categoria: produto.categoria, data, observacao: obs || "AutomÃ¡tico",
  }]).select();
  if (error) { console.log(error); return false; }
  if (!viveiros[index].custos) viveiros[index].custos = [];
  viveiros[index].custos.push({ id: salvo[0].id, tipo: "produto", produtoId: produto.id, nomeProduto: produto.nome, quantidadeG, valor, categoria: produto.categoria, data, observacao: obs || "AutomÃ¡tico" });
  return true;
}
...
async function _aplicarProtocolosRacao(index, racaoKg, data) {
  ...
  for (const p of prots) {
    ...
    const ok = await _lancarCustoAuto(index, produto, quantidadeG, data, "AutomÃ¡tico (raÃ§Ã£o)");
    if (ok) aplicados.push({ nome: produto.nome, quantidadeG, valor: (produto.custoPorGrama || 0) * quantidadeG });
  }
  return aplicados;
}

// Aplica um protocolo de raÃ§Ã£o aos lanÃ§amentos de raÃ§Ã£o jÃ¡ existentes.
// Retorna { n: quantos custos criou, valor: soma, pulados: jÃ¡ existentes, total: raÃ§Ãµes no perÃ­odo }
async function _aplicarProtocoloRacaoRetroativo(index, prot) {
  ...
  for (const r of racoes) {
    ...
    const jaTem = (v.custos || []).some(c => c.data === r.data && c.produtoId === produto.id && (c.observacao || "").startsWith("AutomÃ¡tico"));
    if (jaTem) { pulados++; continue; }
    const quantidadeG = (Number(prot.dosePorKgG) || 0) * r.racao;
    const ok = await _lancarCustoAuto(index, produto, quantidadeG, r.data, "AutomÃ¡tico (raÃ§Ã£o)");
    if (ok) { n++; valor += (produto.custoPorGrama || 0) * quantidadeG; }
  }
  return { n, valor, pulados, total: racoes.length };
}

// PÃµe em dia os protocolos semanais ao abrir o app
    ...
    for (const p of prots) {
      ...
      while (cur <= fim && guard < 400) {
        ...
        if (p.dias.includes(cur.getDay())) {
          const jaTem = (v.custos || []).some(c => c.data === ds && c.produtoId === produto.id && (c.observacao || "").startsWith("AutomÃ¡tico"));
          if (!jaTem) await _lancarCustoAuto(index, produto, Number(p.quantidadeG) || 0, ds, "AutomÃ¡tico (semanal)");
        }
...
function _maResumoProtocolo(p) {
  ...
  const desde = p.inicio ? ` Â· desde ${formatarData(p.inicio)}` : "";
  if (p.tipo === "racao") {
    const doseTxt = (p.doseModo === "pct" && p.dosePct)
      ? `${formatarNumeroBR(p.dosePct, 2)}% da raÃ§Ã£o`
...
function abrirManejoAutomatico(index) {
  ...
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
        ...
        ${prots.length === 0 ? `<p class="ma-vazio">Nenhum protocolo configurado.</p>` : prots.map(p => {
          ...
          return `<div class="ma-item ${p.ativo ? "" : "ma-inativo"}">
            <div class="ma-item-info">
              ...
              <span class="ma-item-regra">${p.tipo === "racao" ? "Atrelado Ã  raÃ§Ã£o" : "Programado semanal"} Â· ${_maResumoProtocolo(p)}</span>
            </div>
      ...
      ${produtos.length > 0 ? `<button class="botao-salvar" style="margin-top:12px" onclick="abrirFormProtocolo(${index})"><svg viewBox="0 0 24 24" style="width:18px;height:18px;stroke:white;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Adicionar produto automÃ¡tico</button>` : ""}
...
function abrirFormProtocolo(index, protId) {
  ...
  area.innerHTML = `
    ...
    <div class="cfg-wrap">
      ...
      <div class="campo-form">
        <div class="campo-label"><svg class="campo-icone" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M12 2v4M12 18v4M2 12h4M18 12h4"/></svg><label>Tipo de lanÃ§amento</label></div>
        <select id="protTipo" onchange="_protToggleTipo()">
          <option value="racao" ${tipo === "racao" ? "selected" : ""}>Atrelado Ã  raÃ§Ã£o (por kg)</option>
      ...
      <div id="prot-racao" style="display:${tipo === "racao" ? "block" : "none"}">
        <div class="campo-form">
          ...
          <select id="protDoseModo" onchange="_protToggleDose()">
            <option value="gkg" ${_protDoseModo !== "pct" ? "selected" : ""}>Gramas por kg de raÃ§Ã£o (g/kg)</option>
            <option value="pct" ${_protDoseModo === "pct" ? "selected" : ""}>Porcentagem da raÃ§Ã£o (%)</option>
          </select>
        </div>
        <div class="campo-form" id="prot-dose-gkg" style="display:${_protDoseModo === "pct" ? "none" : "block"}">
          <div class="campo-label"><svg class="campo-icone" viewBox="0 0 24 24"><path d="M3 11h18M5 11a7 7 0 0 0 14 0"/></svg><label>Dose por kg de raÃ§Ã£o (g)</label></div>
          ...
          <p class="rc-print-dica">Ex.: 5 g de produto para cada kg de raÃ§Ã£o lanÃ§ada.</p>
        </div>
        <div class="campo-form" id="prot-dose-pct" style="display:${_protDoseModo === "pct" ? "block" : "none"}">
          <div class="campo-label"><svg class="campo-icone" viewBox="0 0 24 24"><line x1="19" y1="5" x2="5" y2="19"/><circle cx="6.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg><label>Porcentagem da raÃ§Ã£o (%)</label></div>
          <input type="number" inputmode="decimal" id="protDosePct" step="any" placeholder="Ex: 2" value="${_protDoseModo === "pct" && p ? (p.dosePct ?? "") : ""}">
          <p class="rc-print-dica">Ex.: 2% 2 g de produto para cada 100 g de raÃ§Ã£o.</p>
        </div>
      </div>

      <div id="prot-semanal" style="display:${tipo === "semanal" ? "block" : "none"}">
        <div class="campo-form">
          <div class="campo-label"><svg class="campo-icone" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg><label>Quantidade por aplicaÃ§Ã£o (g)</label></div>
      ...
      <p class="rc-print-dica" id="prot-dias-dica">Atrelado Ã  raÃ§Ã£o: deixe vazio para aplicar sempre que lanÃ§ar raÃ§Ã£o. Programado: selecione os dias.</p>
      ...
      <p class="rc-print-dica">Deixe vazio para valer desde o inÃ­cio do cultivo.</p>

      <label class="ma-check"><input type="checkbox" id="protRetroativo"> Aplicar aos dias anteriores (lanÃ§a o que jÃ¡ passou)</label>
...
async function salvarProtocolo(index, protId) {
  ...
  if (tipo === "racao") {
    ...
    if (modo === "pct") {
      ...
      if (!pct || pct <= 0) { erro("Informe a porcentagem da raÃ§Ã£o."); return; }
      prot.doseModo = "pct";
      prot.dosePct = pct;
      prot.dosePorKgG = pct * 10; // pct% de 1000 g de raÃ§Ã£o = pctÃ—10 g por kg
    } else {
      const dose = parseFloat(document.getElementById("protDosePorKg").value);
      if (!dose || dose <= 0) { erro("Informe a dose por kg de raÃ§Ã£o."); return; }
      prot.doseModo = "gkg";
      prot.dosePct = null;
      prot.dosePorKgG = dose;
    }
    prot.dias = dias; // vazio = todo dia que lanÃ§ar raÃ§Ã£o
  } else {
    const qtd = parseFloat(document.getElementById("protQtd").value);
    if (!qtd || qtd <= 0) { erro("Informe a quantidade por aplicaÃ§Ã£o."); return; }
    if (dias.length === 0) { erro("Selecione ao menos um dia da semana."); return; }
    prot.quantidadeG = qtd;
    prot.dias = dias;
    // Retroativo: backfill desde o inÃ­cio; senÃ£o, comeÃ§a de hoje em diante
  ...
  // Aplica lanÃ§amentos (pode envolver vÃ¡rios custos)
  if (tipo === "semanal") {
    ...
    if (r.n > 0) {
      ...
      setTimeout(() => _toastErro('Nenhuma raÃ§Ã£o anterior encontrada. Confira a data em "Aplicar a partir de" e se hÃ¡ raÃ§Ã£o lanÃ§ada.'), 500);
    }
  }
  abrirManejoAutomatico(index);
}
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        <div class="campo-form">
          ...
          <input type="text" id="nomeProduto" placeholder="Ex: RaÃ§Ã£o">
        </div>
        <div class="campo-form">
          <div class="campo-label">
          ...
          <select id="categoriaProduto">
            ...
            <option value="ProbiÃ³tico">ProbiÃ³tico</option>
...
async function salvarProduto() {
  ...
  if (produtos.some(p => (p.nome || "").trim().toLowerCase() === nomeNorm && p.categoria === categoria)) {
    if (erroProd) { erroProd.textContent = `JÃ¡ existe um produto "${nome}" nessa categoria. Edite o existente em vez de duplicar.`; erroProd.style.display = "block"; }
    return;
  }
  ...
  if (error) {
    ...
    if (erroEl) {
      erroEl.textContent = error.code === "42P01"
        ? "Tabela 'produtos' nÃ£o existe ainda. Crie-a no Supabase conforme instruÃ§Ãµes."
        : "Erro ao salvar: " + error.message;
...
async function excluirProduto(i, botao) {
  ...
  if (!del || del.length === 0) {
    restaurar();
    _toastErro("NÃ£o foi possÃ­vel excluir no banco (falta a polÃ­tica de exclusÃ£o em 'produtos'). Rode o SQL enviado.");
    return;
  }
  produtos.splice(i, 1);
  abrirVerProdutos();
      ...
      <div class="form-corpo">
        ...
        <div class="campo-form">
          ...
          <select id="editCategoriaProduto">
            <option value="RaÃ§Ã£o" ${p.categoria === "RaÃ§Ã£o" ? "selected" : ""}>RaÃ§Ã£o</option>
            <option value="ProbiÃ³tico" ${p.categoria === "ProbiÃ³tico" ? "selected" : ""}>ProbiÃ³tico</option>
            <option value="CalcÃ¡rio" ${p.categoria === "CalcÃ¡rio" ? "selected" : ""}>CalcÃ¡rio</option>
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <span class="form-caption">${abreviarViveiro(viveiro.nome)}</span>
        <h2 class="form-titulo">LanÃ§ar Custo</h2>
      </div>
      <div class="form-corpo">
        <div class="historico-opcoes-grid">
    ...
    area.innerHTML = `
      <div class="form-lancamento">
        <div class="form-topo">
          ...
          <h2 class="form-titulo">LanÃ§ar Produto</h2>
        </div>
        <div class="form-corpo">
          <div class="viveiro-sem-ciclo-msg">
            <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            <span>Nenhum produto cadastrado. VÃ¡ em Custos e Insumos Cadastrar produto primeiro.</span>
          </div>
...
function recalcularValorEditCusto(index, chaveEnc) {
  ...
  if (!prod) return; // sem produto cadastrado, nÃ£o hÃ¡ preÃ§o para recalcular
...
async function salvarCustoProduto(index) {
  ...
  // Limpa o formulÃ¡rio para um novo lanÃ§amento
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        ...
        <div class="campo-form">
          ...
          <input type="text" id="nomeOutroCusto" placeholder="Ex: AnÃ¡lise de Ã¡gua, Energia, TÃ©cnico...">
        </div>
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
...
// Registro de custo de RaÃ§Ã£o (derivado do ciclo ativo OU snapshot de ciclo
// encerrado): gerenciado pelo sistema, nÃ£o editÃ¡vel nas telas de custo.
function _ehCustoRacao(c) {
  return !!(c && (c.derivado || (c.categoria === "RaÃ§Ã£o" && c.nomeProduto === "RaÃ§Ã£o")));
}
...
function renderizarHistoricoCustos(index, elementoId, direto) {
  ...
  resultado.innerHTML = `
    ...
    <div class="custo-grupo-lista">
      ${lista.length === 0
        ? `<p class="sobrevivencia-texto">Nenhum custo lanÃ§ado.</p>`
        : lista.map((g, gi) => {
      ...
      ${rateioFixo > 0 ? `<div class="custo-card custo-card-rateio">
        ...
        <div class="custo-card-info">
          <span class="custo-card-nome">MÃ£o de obra e custos fixos</span>
          <span class="custo-card-qtd">Rateio automÃ¡tico</span>
        </div>
...
function abrirEditarGrupoCusto(index, chaveEnc, elementoId, direto) {
  ...
  // Unidade padrÃ£o: kg, exceto quando a quantidade Ã© menor que 1 kg
  ...
  resultado.innerHTML = `
    ...
    <div class="cfg-wrap">
      ...
      <div class="campo-form">
        ...
        ${prod ? `<p class="rc-print-dica">Recalculado pela quantidade (R$ ${formatarNumeroBR(prod.custoPorGrama * 1000, 2)}/kg). VocÃª ainda pode ajustar o valor na mÃ£o.</p>` : ""}
      </div>
      ...
      <button class="botao-salvar" onclick="salvarEdicaoGrupoCusto(${index},'${chaveEnc}','${elementoId}',${direto})">Salvar alteraÃ§Ãµes</button>
...
async function salvarEdicaoGrupoCusto(index, chaveEnc, elementoId, direto) {
  ...
  if (isNaN(novoValor) || novoValor < 0) { erro("Informe um valor vÃ¡lido."); return; }

  // Quantidade: para produto, respeita o que foi editado; para outros custos, mantÃ©m o que havia
  let somaQtd;
  if (isProduto) {
    const qtdRaw = parseFloat(document.getElementById("editCustoQtd")?.value);
  ...
  // Remove os lanÃ§amentos do grupo e grava um Ãºnico consolidado
...
function confirmarExcluirGrupoCusto(index, gi, chaveEnc, elementoId, direto) {
  ...
  row.innerHTML = `<div class="custo-grupo-conf">
    <span>Excluir todos os lanÃ§amentos deste item?</span>
    ...
    <div class="form-lancamento">
      ...
      <div class="form-corpo">
        ...
        <div class="campo-form">
          ...
          <input type="text" id="nomeEdicaoCusto" value="${custo.nomeProduto}" placeholder="Ex: RaÃ§Ã£o, PÃ³s larva...">
        </div>
        <div class="campo-form">
          <div class="campo-label">
            <svg class="campo-icone" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
...
function imprimirCustos(viveiroIndex) {
  ...
  // Agrupa por produto/nome (igual Ã  tela): uma linha por item
  const grupos = {};
  custos.forEach(c => {
    const chave = _chaveCusto(c);
  ...
  <table>
    ...
    <tbody>
      ${linhas}
      ${rateioFixo > 0 ? `<tr><td>MÃ£o de obra e custos fixos</td><td>rateio</td><td>R$ ${formatarNumeroBR(rateioFixo, 2)}</td></tr>` : ""}
  ...
  area.innerHTML = `
    <div class="form-lancamento">
      <div class="form-topo">
        ...
        <h2 class="form-titulo">HistÃ³rico geral de custos</h2>
      </div>
      <div class="form-corpo">
        <div class="tabela-historico">
          <div class="linha-hist-custo-geral cabecalho">
            ...
            <span class="col-centro">DESCRIÃ‡ÃƒO</span>
...
async function carregarViveiros() {
  ...
  // CORREÃ‡ÃƒO: filtrar por user_id para nÃ£o carregar dados de outros usuÃ¡rios
  ...
  if (erroRacoes) {
    ...
    _erroCarregamento("Erro ao carregar raÃ§Ãµes.");
    return;
  }
  ...
  // Carregar produtos (gracioso se a tabela nÃ£o existir ainda)
  ...
  // Carregar tipos de raÃ§Ã£o (gracioso se a tabela nÃ£o existir ainda)
  ...
  // Carregar custos fixos mensais (gracioso se a tabela nÃ£o existir ainda)
  ...
  // Carregar assinatura do usuÃ¡rio (gracioso se a tabela nÃ£o existir ainda)
  ...
  // Carregar custos (gracioso se a tabela nÃ£o existir ainda)
  ...
  viveiros = viveirosData.map((item) => ({
    ...
    ciclosFinalizados: ciclosData
      ...
      .map((ciclo) => ({
        ...
        // HistÃ³rico persistido (gracioso para ciclos antigos sem esses campos)
  ...
  // Ordenar viveiros por nÃºmero no nome (Viveiro 1, Viveiro 2...)
  viveiros.sort((a, b) => {
    const numA = parseInt(a.nome.replace(/\D/g, "")) || 0;
    const numB = parseInt(b.nome.replace(/\D/g, "")) || 0;
    return numA - numB || a.nome.localeCompare(b.nome, "pt-BR");
  ...
  // Monta o custo de RaÃ§Ã£o derivado (preÃ§o do catÃ¡logo Ã— kg lanÃ§ados no ciclo)
  _montarCustoRacaoVirtual();
...
// de forma idempotente e segura contra concorrÃªncia. NÃ£o toca em ciclos
...
async function _garantirCicloIdViveirosAtivos() {
  ...
  for (const v of semId) {
    if (v.cicloId) continue; // jÃ¡ preenchido nesta execuÃ§Ã£o
    const novo = _novoCicloId();
    // UPDATE condicional: sÃ³ grava se ainda estiver null. Se outra aba/dispositivo
    // preencher primeiro, o filtro nÃ£o casa e retornamos vazio (sem sobrescrever).
    ...
    if (data && data.length && data[0].ciclo_id) {
      ...
      // NinguÃ©m foi atualizado (outro cliente venceu a corrida): relÃª o valor atual
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (session) {
      ...
      // Garante ciclo_id nos viveiros ativos antigos (uma Ãºnica vez, idempotente)
      try { await _garantirCicloIdViveirosAtivos(); } catch (e) { console.log("ciclo_id backfill:", e); }

      // PÃµe em dia os protocolos semanais (lanÃ§amento automÃ¡tico)
...
// Delegado no documento (captura) para valer tambÃ©m em botÃµes criados dinamicamente.
(function () {
